...
class TD3Actor(nn.Module):
    def __init__(self, state_dim, goal_dim, action_dim, scale=None):
        ...
        self.l1 = nn.Linear(state_dim + goal_dim, 64)
...
class HiroAgent(Agent):
    ...
    def step(self, s, env, step, global_step=0, explore=False):
        ...
        # print("goal pos:", env.goal_pos[:2])
        # print("agent pos: ", env.robot_pos[:2])

        n_pos = env.robot_pos[:2]
        if explore:
            if global_step < self.start_training_steps:
                n_sg = self.subgoal.action_space.sample()
            else:
                n_sg = self._choose_subgoal_with_noise(step, s, self.sg, n_s, n_pos)
        else:
            n_sg = self._choose_subgoal(step, s, self.sg, n_s, n_pos)
    ...
    def _choose_subgoal_with_noise(self, step, s, sg, n_s, n_pos):
        # print("sg", sg)
    ...
    def _choose_subgoal(self, step, s, sg, n_s, n_pos):
    ...
    def subgoal_transition(self, s, sg, n_s, n_pos):
        # print("sg: ", sg)
        # print("sg.shape[0]: ", sg.shape[0])
        return s[:sg.shape[0]] + sg - n_pos[:sg.shape[0]]
