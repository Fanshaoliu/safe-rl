...
from safe_rl.pg.agents import PPOAgent, TRPOAgent, CPOAgent, HRLAgent
...
def run_polopt_agent(env_fn, 
    ...
    # state shape: Box(-inf, inf, (104,), float32)
    # action shape: Box(-1.0, 1.0, (12,), float32)
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # print("-----------------")
            # print("a: \n", a)
            # print(np.shape(a))  # (1,12)

            # Step in environment
            o2, r, d, info = env.step(a)
            '''
            经测试，env.step确实来自于engine.py的step函数，函数中有o2的详细定义，简单来说，o2不是图片，而是包含了雷达信息，接触角
            等agent所处物理世界的床干起信息，以此来对物理世界的智能小车做一个仿真
            '''

            # print("o2: ", o2[:5])
            # print(np.shape(o2))  # (104,)
            #
            # print("r: \n", r)
            # print(np.shape(r))  # ()
            #
            # print("info: \n", info)
            '''
            对于v1难度，障碍物只有陷阱，因此info只有这两项：
            {'cost_hazards': 0.0, 'cost': 0.0}
            对于v2难度，障碍物多了易碎物体，info有四项：
            {'cost_vases_contact': 0.0, 'cost_vases_velocity': 0.0, 'cost_hazards': 0.0, 'cost': 0.0}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Safexp-CarPush1-v0')
